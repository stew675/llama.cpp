    ...
    void accept(llama_seq_id seq_id, uint16_t n_accepted, bool is_other) override {
        ...
        if (adaptive && (!is_other || n_accepted >= adaptive_ctrl[seq_id].n_cur)) {
            ...
            if (adaptive_ctrl[seq_id].n_cur != depth_before) {
                SPC_TRC("adaptive draft depth seq %d: %d -> %d (n_draft=%d, n_accepted=%d%s)\n",
                        (int) seq_id, depth_before, adaptive_ctrl[seq_id].n_cur,
                        is_other ? n_accepted : n_last[seq_id], n_accepted,
                        is_other ? ", other" : "");
            }
        }
