...
// only for the staged WMMA operands. Near-lossless, NOT bit-exact -- the env gate
// GGML_CUDA_GDN_CHUNKED_BF16=1 selects it; the bit-exact fp32 chunked path stays the default.
//
...
template <int HG_RATIO>
static void launch_gdn_bf16_kkt(
    ...
    ggml_cuda_kernel_launch(gdn_bf16_kkt_cuda<HG_RATIO>, launch_params,
        k_d, g_d, b_d, A_sc, H, H_k, n_tokens, n_seqs, sq1, sq2, sq3, dbg);
}

static void launch_gdn_bf16_scan(
    ...
    const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(grid, block, 0, stream);
    ggml_cuda_kernel_launch(gdn_bf16_scan_cuda, launch_params,
...
void ggml_cuda_op_gated_delta_net_chunked_bf16(ggml_backend_cuda_context & ctx, ggml_tensor * dst, float * state_d_ext) {
    ...
    switch (hg_ratio) {
        case 1:  launch_gdn_bf16_kkt<1>(k_d, g_d, b_d, A_sc.get(), H, neqk1, n_tokens, n_seqs, sq1, sq2, sq3, stream, dbg); break;
        case 2:  launch_gdn_bf16_kkt<2>(k_d, g_d, b_d, A_sc.get(), H, neqk1, n_tokens, n_seqs, sq1, sq2, sq3, stream, dbg); break;
        case 3:  launch_gdn_bf16_kkt<3>(k_d, g_d, b_d, A_sc.get(), H, neqk1, n_tokens, n_seqs, sq1, sq2, sq3, stream, dbg); break;
        case 4:  launch_gdn_bf16_kkt<4>(k_d, g_d, b_d, A_sc.get(), H, neqk1, n_tokens, n_seqs, sq1, sq2, sq3, stream, dbg); break;
        case 6:  launch_gdn_bf16_kkt<6>(k_d, g_d, b_d, A_sc.get(), H, neqk1, n_tokens, n_seqs, sq1, sq2, sq3, stream, dbg); break;
        case 8:  launch_gdn_bf16_kkt<8>(k_d, g_d, b_d, A_sc.get(), H, neqk1, n_tokens, n_seqs, sq1, sq2, sq3, stream, dbg); break;
    ...
    launch_gdn_bf16_scan(q_d, k_d, v_d, g_d, b_d, s_d, dst_d, state_d, A_sc.get(),
